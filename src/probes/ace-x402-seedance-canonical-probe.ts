/**
 * Ace Data x402 probe for Seedance (ByteDance) video generation — using
 * the CANONICAL envelope from Ace Data's reference X402Client repo.
 *
 * Seedance is the third of Doms's recommended stable video providers
 * (Kling, Veo, Seedance) and was untested in our project. Per
 * SKILL_acedatacloud_mcps.md the default model is
 * doubao-seedance-1-0-pro-250528, default 720p/16:9/5s.
 *
 * Endpoint pattern follows the standard Ace Data convention:
 *   POST /seedance/videos to create, GET /seedance/tasks/{id} to poll.
 *
 * Setup (if not already done):
 *   npm install undici
 *
 * Run:
 *   tsx src/probes/ace-x402-seedance-canonical-probe.ts
 *
...
// Per MCP defaults. If 400 says missing/wrong field, see error and adjust.
const REQUEST_BODY = {
  model: 'doubao-seedance-1-0-pro-250528',
  prompt: 'A short 6-second cinematic test clip: dark abstract motion, seamless loop, no text.',
  // optional fields documented in MCP — uncomment if minimal body 400s:
  // resolution: '720p',
  // ratio: '16:9',
  // duration: 5,
};
...
async function main() {
  ...
  console.log('=== Ace Data x402 Seedance probe (canonical envelope) ===');
  ...
  console.log();
  if (res1.status === 404) {
    console.log('Got 404. /seedance/videos may not be the correct endpoint.');
    console.log('Check the Ace Data Seedance docs for the actual path.');
    return;
  }
  if (res1.status !== 402) {
  ...
  if (res2.status !== 200) {
    console.log('=== NON-200 ===');
    console.log(`Seedance /videos returned ${res2.status}.`);
    if (body2.toLowerCase().includes('field') || body2.toLowerCase().includes('required')) {
      console.log('Body may need additional fields. Uncomment resolution/ratio/duration in REQUEST_BODY.');
    }
    return;
  }
